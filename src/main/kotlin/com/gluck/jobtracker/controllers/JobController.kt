...
class JobController(private val service: JobService) {
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Job Applications Found"),
    ])
    @GetMapping("/jobs")
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "201", description = "Job Applications created", content = [
            Content(mediaType = "text/plain", schema =
                Schema(type = "string", example = "/api/jobs/3"))]),
        ApiResponse(responseCode = "400", description = "Check request correctness"),
        ApiResponse(responseCode = "403", description = "Unauthorized, login to process request")
    ])
    @PostMapping("/jobs")
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Job Application by provided id found", content = [
            Content(mediaType = "application/json",
                schema = Schema(implementation = JobApplicationResponse::class))
        ]),
        ApiResponse(responseCode = "404", description = "Job Application by provided id not found", content = [
            Content(mediaType = "application/json",
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Job Application by provided id is found and updated",
